        ...
        <div className="relative z-10 mx-auto flex min-h-[72vh] w-full min-w-0 max-w-7xl flex-col justify-end px-4 pb-12 pt-24 sm:px-6 sm:pb-16 sm:pt-28 lg:px-8">
          ...
          <div className="mt-8 flex flex-wrap items-end justify-between gap-8">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-5 display-type text-3xl tracking-tight text-paper sm:text-6xl">
                {project.title}
              </h1>
          ...
          {(project.metrics.length ? project.metrics : project.results.slice(0, 4).map((result) => ({ value: result, label: "Outcome" }))).map(
            (metric) => (
              <div key={metric.label + metric.value} className="bg-ink px-4 py-6 sm:px-6 sm:py-8">
                <div className="display-type text-3xl text-accent-blue sm:text-4xl">{metric.value}</div>
        ...
        <div className="min-w-0">
          <MotionReveal>
            <div id="overview" className="grid gap-8 sm:grid-cols-3">
              ...
              <div className="rounded-sm border border-border bg-surface p-5">
                {/* projectIcon returns a stable reference from a static lookup table, not a new component. */}
          ...
          <div id="challenge" className="mt-20 grid gap-10 lg:grid-cols-2">
            <MotionReveal>
              ...
              <h2 className="mt-3 display-type text-3xl">The challenge</h2>
          ...
          <MotionReveal>
            <div id="approach" className="mt-20">
              ...
              <h2 className="mt-3 display-type text-3xl">How we got there</h2>
              <ol className="mt-8 flex flex-col gap-4">
                {project.approach.map((step, i) => (
                  <li key={step} className="flex gap-4 rounded-sm border border-border bg-surface p-5">
          ...
          <MotionReveal>
            <div id="results" className="mt-20">
              ...
              <h2 className="mt-3 display-type text-3xl">What changed</h2>
              <ul className="mt-8 grid gap-3 sm:grid-cols-2">
                {project.results.map((result) => (
                  <li key={result} className="flex items-start gap-3 rounded-sm border border-border p-4">
            ...
            <div id="gallery" className="mt-20">
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                {project.gallery.map((item) => (
                  <figure key={item.url} className="overflow-hidden rounded-sm">
      ...
      {related.length > 0 && (
        <section className="border-t border-border py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 className="display-type text-2xl">More {project.category}</h2>
            <div className="mt-8 grid gap-6 sm:grid-cols-2">
              {related.map((item) => (
                <Link key={item.slug} href={`/projects/${item.slug}`} className="group overflow-hidden rounded-sm border border-border">
                  ...
                  <div className="p-5">
                    <h3 className="text-lg font-medium group-hover:text-accent-indigo">{item.title}</h3>
